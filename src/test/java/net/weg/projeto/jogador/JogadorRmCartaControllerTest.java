...
public class JogadorRmCartaControllerTest {
    ...
    @Test
    public void rmCarta_retornaString() throws Exception {
        Carta carta = new Carta(CartaPadraoEnum.GAMBA);
        Jogador jogador = new Jogador("nome", "senha", 0, null);
        String retorno = "Removeu!";

        when(jogadorservice.rmCarta(any(), any())).thenReturn(retorno);
        when(jogadorservice.readOne(any())).thenReturn(jogador);
        when(cartaService.readOne(any())).thenReturn(carta);

        mockMvc.perform(put("/jogador/{nomeJogador}/rmCarta/{nomeCarta}", jogador.getNome(), carta.getNome())
